...
def get_ws_liquidations(symbol, exchange='binance'):
    with ws_lock:
        ...
        if key not in ws_liquidation_data:
            return 0.0, 0.0, 0.0
        data = ws_liquidation_data[key]
        ...
        try:
            async with websockets.connect(uri) as websocket:
                ...
                subscribe_msg = json.dumps({
                    "op": "subscribe",
                    "args": [
    "liquidation.BTCUSDT", "liquidation.ETHUSDT", "liquidation.SOLUSDT",
    "liquidation.BNBUSDT", "liquidation.XRPUSDT", "liquidation.DOGEUSDT",
    "liquidation.ADAUSDT", "liquidation.AVAXUSDT", "liquidation.LINKUSDT",
    "liquidation.DOTUSDT"
]  # или конкретные пары
                })
                ...
                while ws_running:
                    try:
                        ...
                        if 'topic' in data and data['topic'].startswith('liquidation'):
                            order = data['data']
                            symbol = order['symbol']
    ...
    def monitor():
        ...
        while active_monitors.get(chat_id, False):
            try:
                ...
                reset_daily_counters(chat_id)
                # Сбрасываем WS ликвидации каждый цикл
                with ws_lock:
                    for key in list(ws_liquidation_data.keys()):
                        ws_liquidation_data[key] = {'long': 0.0, 'short': 0.0, 'total': 0.0, 'last_update': time.time()}
