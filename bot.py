...
if __name__ == "__main__":
    ...
    bot.infinity_polling()import json
from collections import defaultdict
from datetime import datetime, timedelta, timezone
import os
import logging
...
def get_main_keyboard():
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    buttons = ["📈 Цена BTC", "🔍 Анализ BTC", "🧠 Автосигналы", "📡 Статус мониторинга", "⚙ Настроить сигналы", "ℹ️ Помощь"]
    keyboard.add(*buttons)
    return keyboard
...
def send_welcome(message):
    ...
    bot.send_message(chat_id, "🤖 Бот готов к работе. Настройте параметры и запустите мониторинг.", reply_markup=get_main_keyboard())
...
@bot.message_handler(func=lambda msg: msg.text in ["💰 Порог цены", "📊 Порог объема", "📈 Порог OI", "⏱ Интервал анализа", "💧 Порог ликвидаций"])
def setting_selection(message):
    chat_id = message.chat.id
    text = message.text
    params = {
        ...
        while active_monitors.get(chat_id):
            try:
                for exch in exchanges:
                    ...
                    with ThreadPoolExecutor(max_workers=20) as executor:
                        futures = {executor.submit(analyze_market, s, settings.get('interval', "15"), settings.get('enable_liq'), exch): s for s in symbols}
...
def price_btn(message):
    data = get_binance_ticker("BTCUSDT")
    if data:
        bot.send_message(message.chat.id, f"💰 <b>BTC/USDT</b>: {data['price']}$ ({data['price_change_24h']:+.2f}%)", parse_mode='HTML')
